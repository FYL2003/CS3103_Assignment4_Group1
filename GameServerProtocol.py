...
class GameServerProtocol(QuicConnectionProtocol):
    ...
    async def _deliver_reliable(self):
        ...
        if (
            self.expected_seq not in self.reliable_buffer
            and self.expected_seq <= self.reliable_seq_largest
        ):
    ...
    async def _deliver_packet(self, data, reliable, seq_no, timestamp):
        ...
        self.metrics[RELIABLE if reliable else UNRELIABLE]["last_latency"] = latency

        # update only if seq_no is higher than previous highest, may be out of order
        self.metrics[RELIABLE]["last_proper_seq"] = max(
            self.metrics[RELIABLE]["last_proper_seq"], seq_no
        )
        self.metrics[UNRELIABLE]["last_proper_seq"] = max(
            self.metrics[UNRELIABLE]["last_proper_seq"], seq_no
        )
    ...
    def _update_metrics(self, channel, packet_size, timestamp):
        ...
        # Jitter calculation
        last_latency = ch_metrics["last_latency"]
        latency = int(time.time() * 1000) - timestamp
        ch_metrics["latency_samples"].append(latency)
        if last_latency is not None:
            ch_metrics["jitter_samples"].append(abs(latency - last_latency))
    ...
    def print_statistics(self):
        ...
        for channel, name in [(RELIABLE, "RELIABLE"), (UNRELIABLE, "UNRELIABLE")]:
            ...
            if channel is RELIABLE:
                pdr = (
                    ch_metrics["packets_received"] / (self.reliable_seq_largest + 1)
                ) * 100
            else:
                pdr = (
                    ch_metrics["packets_received"] / (self.unreliable_seq_largest + 1)
                ) * 100

            avg_latency = (
                sum(ch_metrics["latency_samples"]) / len(ch_metrics["latency_samples"])
                if ch_metrics["latency_samples"]
                else 0
            )
            avg_jitter = (
                sum(ch_metrics["jitter_samples"]) / len(ch_metrics["jitter_samples"])
                if ch_metrics["jitter_samples"]
                else 0
            )
